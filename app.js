...
const debug = require('debug')('egg:plugin:cuteI18n');

/**
 * cuteI18n 国际化
 *
 * 通过设置 Plugin 配置 `cuteI18n: true`，开启多语言支持。
 *
 * #### 语言文件存储路径
 *
...
module.exports = app => {
  /**
   * 如果开启了 cuteI18n 多语言功能，那么会出现此 API，通过它可以获取到当前请求对应的本地化数据。
   *
   * 详细使用说明，请查看 {@link cuteI18n}
   ...
   */
  app.config.cuteI18n.functionName = '__';

  /* istanbul ignore next */
  app.config.cuteI18n.dirs = Array.isArray(app.config.cuteI18n.dirs) ? app.config.cuteI18n.dirs : [];
  ...
  for (const unit of app.loader.getLoadUnits()) {
    ...
    if (fs.existsSync(localePath)) {
      app.config.cuteI18n.dirs.push(localePath);
    } else {
      app.config.cuteI18n.dirs.push(path.join(unit.path, 'config/locales'));
    }
  }

  debug('app.config.cuteI18n.dirs:', app.config.cuteI18n.dirs);
  locales(app, app.config.cuteI18n);
  ...
  // 自动加载 Middleware
  app.config.coreMiddleware.push('cuteI18n');
};
